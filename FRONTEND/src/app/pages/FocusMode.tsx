...
export const FocusMode = () => {
  ...
  const [currentApp, setCurrentApp] = useState("VS Code"); // For distraction detection
  ...
  const startSession = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5000/session/start", {
        ...
        body: JSON.stringify({ duration: timeLeft })
      });
  ...
  return (
    <>
      ...
      <DistractionMonitor
        ...
        onTaskConfig={(config) => console.log("Task config saved:", config)}
      />
