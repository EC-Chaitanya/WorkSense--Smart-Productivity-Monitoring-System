...
export const Analytics = () => {
  ...
  const trendData = analyticsData?.weekly_trends || defaultTrendData;
  const timeDistribution = analyticsData?.time_distribution 
    ? [
        { name: "Deep Work", value: analyticsData.time_distribution.DeepWork || analyticsData.time_distribution["Deep Work"] || 55 },
        { name: "Meetings", value: 20 },
        { name: "Breaks", value: analyticsData.time_distribution.Breaks || 15 },
        { name: "Admin", value: analyticsData.time_distribution.Admin || 10 },
      ]
    : defaultTimeDistribution;
  const appUsage = analyticsData?.top_apps || defaultAppUsage;

  // Calculate average score
  const averageScore = trendData.length > 0
    ? Math.round(trendData.reduce((sum, week) => sum + week.score, 0) / trendData.length)
    : 85;

  // Calculate metric values
  const deepWorkHours = analyticsData?.time_distribution?.["Deep Work"] 
    || analyticsData?.time_distribution?.DeepWork 
    || 32;
  const deepWorkDisplay = `${Math.floor(deepWorkHours)}h ${Math.round((deepWorkHours % 1) * 60)}m`;
  
  const breakHours = analyticsData?.time_distribution?.Breaks || 6;
  const breakDisplay = `${Math.floor(breakHours)}h ${Math.round((breakHours % 1) * 60)}m`;
  
  const adminHours = analyticsData?.time_distribution?.Admin || 4;
  const adminDisplay = `${Math.floor(adminHours)}h ${Math.round((adminHours % 1) * 60)}m`;
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 pb-12 font-sans tracking-tight">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        >
          {loading ? (
            <div className="animate-spin rounded-full h-56 w-56 border-4 border-slate-200/20 dark:border-white/10 border-t-[#5B8CFF]" />
          ) : (
            <AnimatedScoreRing score={averageScore} />
          )}
        ...
        <div className="col-span-1 lg:col-span-2 grid gap-6 sm:grid-cols-2">
          <MetricCard key="metric-1" title="Deep Work Hours" value={deepWorkDisplay} icon={Clock} trend="+4.2%" positive delay={0.1} />
          <MetricCard key="metric-2" title="Distraction Time" value="4h 30m" icon={ZapOff} trend="-1.5%" positive delay={0.2} />
          <MetricCard key="metric-3" title="Break Time" value={breakDisplay} icon={Zap} trend="+0.8%" positive delay={0.3} />
      ...
      >
        ...
        <DistractionActions
          ...
          onActionClick={(actionId) => {
            ...
            if (actionId.includes("focus") || actionId.includes("strict")) {
